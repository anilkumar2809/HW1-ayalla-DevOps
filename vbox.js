...
class VBoxProvider {
    ...
    async postconfiguration(name)
    {
        ...
        await sshExec("ls /", this.sshConfig);
        await sshExec("sudo apt-get update", this.sshConfig);
        await sshExec("sudo apt-get install -y nodejs", this.sshConfig);
        await sshExec("sudo apt-get install -y npm", this.sshConfig);
        await sshExec("sudo apt-get install git", this.sshConfig);
        await sshExec("git clone https://github.com/CSC-DevOps/App", this.sshConfig);
        await sshExec("cd App", this.sshConfig);
        await sshExec("npm install", this.sshConfig);
        await sshExec("npm install express", this.sshConfig);
